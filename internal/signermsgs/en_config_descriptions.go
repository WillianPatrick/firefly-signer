...
var (
	...
	// <Implement 2024-11-07 - Willian Patrick dos Santos - superhitec@gmail.com>
	SecurityPrivateAddressKey = ffc("config.security.anonymizedAddressKey", "Local key used for data encryption to ensure privacy", "string")

	// AWS KMS Configurations
	ConfigAWSWalletEnabled         = ffc("config.awsWallet.enabled", "Enable or disable AWS Wallet integration", "boolean")
	ConfigAWSWalletRegion          = ffc("config.awsWallet.region", "AWS region where KMS and Secrets Manager keys are located", "string")
	ConfigAWSWalletAccessKeyID     = ffc("config.awsWallet.accessKeyID", "AWS Access Key ID for authentication", "string")
	ConfigAWSWalletSecretAccessKey = ffc("config.awsWallet.secretAccessKey", "AWS Secret Access Key for authentication", "string")
	ConfigAWSWalletUseSecrets      = ffc("config.awsWallet.useSecrets", "Enable or disable retrieval of keys from Secrets Manager for each address", "boolean")
	ConfigAWSWalletUseKMS          = ffc("config.awsWallet.useKMS", "Enable or disable AWS KMS for key management operations", "boolean")
	ConfigAWSWalletEncryptSecrets  = ffc("config.awsWallet.encryptSecrets", "Enable encryption for secrets in storage", "boolean")

	// MongoDB Configurations
	ConfigMongoDBConnectionString = ffc("config.mongodb.connectionString", "Connection string for MongoDB, used to store wallet data", "string")
	ConfigMongoDBDataBaseName     = ffc("config.mongodb.databaseName", "Database name within MongoDB to use for storing wallet data", "string")
	ConfigMongoDBCollectionName   = ffc("config.mongodb.collectionName", "Collection name within MongoDB to use for storing wallet records", "string")
	// </Implement>
)
